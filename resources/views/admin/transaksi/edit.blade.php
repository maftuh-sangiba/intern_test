...
<div class="card">
  <div class="card-body">
    <form>
      ...
      <h5>Total Price: <span id="totalPrice">{{ $transaksi->total }}</span></h5>
      <button class="btn btn-success btn-sm mt-3" id="submitData">Edit Transaksi</button>
    </form>
  </div>
</div>
  ...
  $(document).ready(function () {
    ...
    $('#submitData').on('click', function() {
      ...
      selectedProductsTable.find('tbody tr').each(function () {
        ...
        const price = parseFloat($(this).find('td:nth-child(4)').text());
        const detailId = $(this).find('td:nth-child(5)').text();
      ...
      $.ajax({
        url: `{{ route('app.transaksi.update', $transaksi->id) }}`,
        method: 'PUT',
        data: {
          _token: "{{ csrf_token() }}",
